...
public class PartialSum_2 {
	...
	public static void main(String[] args) {
		...
		scanner.nextLine();
		int[] arr = new int[n]; // 수열 입력받을 빈 배열
		int check = 0;
		...
		for (int q = 0; q < n; q++) { // 부분합 비교를 위한 for문
			...
			for (int r = q; r < n; r++) {
				sum += arr[r]; // q부터 차례대로 한개씩 합하고
				if (sum >= s) { // 그 합이 비교값 이상이 되면
					count = r - q + 1; // 그 때 연속된 수열의 개수를 저장하고
					break; // 중첩 for문을 종료하고 다음 q에 대해서 같은 작업 진행
				}
			}
			if(check == 0 && count != 0) { // check가 0이면서 count가 0이 아니면 count를 check에 저장
				check = count;
			} else if(check != 0 && count != 0) { // check가 0이 아니면서 count가 0이 아니면 check와 count를 비교해서 더 작은 수를 count에 저장
				check = (check > count) ? count : check;
			}
		}
		System.out.println(check); // 조건에 맞는 check를 출력
	}
